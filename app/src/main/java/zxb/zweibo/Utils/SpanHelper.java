...
/**
 * 把新浪微博的文本内容转换为SpannableString的类
 * Created by rex on 15-8-14.
 */
public class SpanHelper {

    Context mContext;
    private EmotionUtil emoUtil;
    private final String LINK = " 网页链接 ";

    public SpanHelper(Context context) {
        this.mContext = context;
        this.emoUtil = new EmotionUtil(context);
    }

    /**
     * 传入字符串，就可以获取对应的SpannableString格式化后的对象.
     * @param text  String
     * @return SpannabelString，可以直接setText上去
     */
    public SpannableString newSpanInstance(String text) {
    ...
    private void emotion(SpannableString spanText) {
        ...
        while (matcher.find()){
            ...
            byte[] emotion = emoUtil.getEmotion(group);
            if (emotion != null){
